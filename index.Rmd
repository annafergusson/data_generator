...
function makeSpin(){
  ...
  spin(canvas, context, 0, randAngle + 4*Math.PI, 0.1, randOutcome)
}

function spin(canvas, context, angle, max, speed, randOutcome){
  if(angle < max)
  {
    drawPointer(canvas, context, angle);
    var wait = 10;
    setTimeout(function(){
      spin(canvas, context, angle + speed, max, speed, randOutcome)
    }, wait)
  }
  else
  {
  ...
  setTimeout(function(){
   ...
   $("#outcome").text(randOutcome);
   var data = [{"outcome":randOutcome}];
   doStuff(data);  
  }
}
